...
- `recordTradeConfirmed(tradeGuid)` - optional marker helper; not required for correctness because `finalizeLoan` already validates `TradeConfirmed`. Keeping it allows early on-chain marking (`tradeConfirmed/collateralActivated`) before finalization; removing it reduces API surface but does not break core loan-finalization safety.
