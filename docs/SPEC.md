...
- The baseline RFQ is EIP-712 signed by an address in `RFQ_SIGNER_ROLE` and must match the pending deposit terms (`collateralAsset`, `collateralAmount`, `maturity`, `borrowAmount`, `loanId`). On the atomic origination paths it must also match the deposit request `putStrike`; after mandate expiry, direct `acceptMandate` refreshes may update `putStrike` to a fresh keeper-signed market bound.
