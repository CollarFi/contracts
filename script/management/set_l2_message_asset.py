...
def _resolve_env_path(env_profile: str, l1_env_file: Path) -> Path:
    profile = env_profile.strip().lower()
    if profile and l1_env_file == (ROOT_DIR / ".env.l1.testnet"):
        return ROOT_DIR / f".env.l1.{profile}"
    return l1_env_file
...
def main(
    ...
    l2_asset: str = typer.Option("", "--l2-asset", help="L2 wrapped asset for LZ payload"),
    vault: str = typer.Option("", "--vault", help="Override L1 vault address"),
    broadcast: bool = typer.Option(False, help="Send onchain tx"),
    json_out: bool = typer.Option(False, "--json"),
) -> None:
    l1_env_file = _resolve_env_path(env_profile, l1_env_file)
    ...
    if not l1_asset or not l2_asset:
        raise ValueError("set --l1-asset/--l2-asset (or WETH_ASSET/L2_WRAPPED_WETH_ASSET in env)")
