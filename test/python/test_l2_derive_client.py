...
class DeriveClientTests(unittest.TestCase):
    ...
    def test_private_withdraw_uses_wrapped_deposit_underlying_for_metadata(self) -> None:
        metadata_assets: list[str] = []

        def fake_decimals(rpc_url: str, asset: str) -> int:
            metadata_assets.append(asset)
            return 18

        def fake_asset_name(fallback: str, asset: str, rpc_url: str) -> str:
            metadata_assets.append(asset)
            return "ETH"

        with (
            patch("ops.management.handlers.l2_derive_client.fresh_action_nonce_and_expiry", return_value=(333, 444)),
            patch("ops.management.handlers.l2_derive_client._metadata_asset", return_value="0x9999999999999999999999999999999999999999"),
            patch("ops.management.handlers.l2_derive_client._erc20_decimals", side_effect=fake_decimals),
            patch("ops.management.handlers.l2_derive_client.resolve_asset_name", side_effect=fake_asset_name),
            patch(
                "ops.management.handlers.l2_derive_client.resolve_tsa_action_config",
                return_value={
                    "depositModule": "0x2222222222222222222222222222222222222222",
            ...
            patch("ops.management.handlers.l2_derive_client.wallet_sign", side_effect=["0xactionsig", "0xauthsig"]),
            patch(
                "ops.management.handlers.l2_derive_client.http_post_json",
                side_effect=[
                    (200, {"result": {"typed_data_hash": "0xtypedhash"}}),
                    (200, {"id": "req-2", "result": {"status": "requested"}}),
                ],
            ),
        ):
            submit_api_for_pending_message(
                action_type=ACTION_RETURN_REQUEST,
                pending_message=self._pending_message(),
                tsa_addr="0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                account="keeper",
