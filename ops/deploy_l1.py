...
def main(
    ...
    verify: bool = typer.Option(False, help="Verify deployed contracts after broadcast"),
    ...
    deployer = resolve_signer(
        ...
        SignerInput(
            ...
            unlocked=unlocked or str(l1.get("UNLOCKED", "")).lower() in {"1", "true", "yes"},
    ...
    proxy_admin = resolve_signer(
        ...
        SignerInput(
            ...
            private_key=proxy_admin_private_key or l1.get("PROXY_ADMIN_PRIVATE_KEY", ""),
            from_addr=proxy_admin_from or l1.get("PROXY_ADMIN_FROM", ""),
            unlocked=proxy_admin_unlocked or str(l1.get("PROXY_ADMIN_UNLOCKED", "")).lower() in {"1", "true", "yes"},
    ...
    if _has_nonzero_addr(l1.get("WETH_ASSET", "")) and not _has_nonzero_addr(l1.get("L2_WRAPPED_WETH_ASSET", "")):
        ...
        print("[cyan][info][/cyan] resolved L2_WRAPPED_WETH_ASSET from L2 TSA:", l1["L2_WRAPPED_WETH_ASSET"])

    if not _has_nonzero_addr(l1.get("L2_RECIPIENT", "")):
        l1["L2_RECIPIENT"] = resolve_l2_receiver_from_env_file(l2_env_file)
        print("[cyan][info][/cyan] resolved L2_RECIPIENT from L2 deployment/env:", l1["L2_RECIPIENT"])
    ...
    verification = VerificationConfig(
        ...
        etherscan_api_key=etherscan_api_key or l1.get("ETHERSCAN_API_KEY", ""),
    )
    if verification.enabled and not (verification.verifier or verification.verifier_url or verification.etherscan_api_key):
        print("[yellow][warn][/yellow] verify requested without explicit verifier params; forge defaults will be used")
