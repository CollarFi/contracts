...
from management.handlers.l2_derive_client import decimal_1e18_to_int, normalize_decimal_str, submit_with_retries  # noqa: E402
...
def process_rfq_trade_queue(runtime: Any, *, state: dict[str, Any], handled: list[dict[str, Any]]) -> tuple[int, int, bool]:
    attempts = 0
    sent = 0
    queue_blocked = False

    while attempts < runtime.max_per_tick and state["rfqTradeQueue"]:
            ...
            if trade.get("executeQuote"):
                if execute_state is None:
                    execute_meta, api_attempt = submit_with_retries(
                        lambda: _sign_and_submit_rfq_execute_quote(
    ...
    return attempts, sent, queue_blocked
